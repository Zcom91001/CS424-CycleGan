...
def main():
    ...
    # Loss weights
    lambda_cycle = 30   # cycle-consistency
    ...
    # Load best checkpoint generators for final evaluation (if available).
    if best_epoch > 0 and os.path.exists(best_ckpt_path):
        checkpoint = torch.load(best_ckpt_path, map_location='cuda' if cuda else 'cpu')
        G_AB.load_state_dict(checkpoint['G_AB'])
        G_BA.load_state_dict(checkpoint['G_BA'])
        print(f'Loaded best checkpoint for evaluation: {best_ckpt_path}')
    else:
        print('Best checkpoint not found; using final in-memory model for evaluation.')
    # ── Evaluation ────────────────────────────────────────────────────────────
