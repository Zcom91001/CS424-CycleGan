...
def main():
    ...
    train_transforms = transforms.Compose([
        ...
        transforms.RandomHorizontalFlip(),           # Fix #9
        transforms.ToTensor(),
    ...
    for epoch in range(n_epoches):
        ...
        for i, (real_A, real_B) in enumerate(trainloader):
            ...
            # Fix #1: identity loss — G_BA preserves domain A; G_AB preserves domain B
            # (original code had the generators swapped, actively harming style transfer)
            loss_id_A = criterion_identity(G_BA(real_A), real_A)
            loss_id_B = criterion_identity(G_AB(real_B), real_B)
            loss_identity = (loss_id_A + loss_id_B) / 2

            # GAN loss (logits → ones)
            ...
            # Fix #8: gradient clipping — prevents exploding gradients that cause blur/artifacts
            ...
            fake_A_buf  = fake_A_buffer.push_and_pop(fake_A_img.detach())
            ...
            fake_B_buf  = fake_B_buffer.push_and_pop(fake_B_img.detach())
